...
class Director {
  ...
  void loadTriangles() {
    ...
    if (xml!=null && points.size()>0) {
      ...
      for (int i = 0; i < children.length; i++) {
        ...
        XML[] sensores = children[i].getChildren("sensor");
        for ( int k=0; k<sensores.length; k++ ) {
          int idSensor = sensores[k].getInt("id");
          aux.agregarIDSensor( idSensor );
        }
  ...
  int revisarTriangulosActivados() {
    int cuantos = 0;
    for ( Triangle t : triangles ) {
      if ( t.conSensoresActivados ) {
        cuantos++;
      }
    }
    return cuantos;
